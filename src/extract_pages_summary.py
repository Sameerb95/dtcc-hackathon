...
# /Users/shirsama/dtcc-hackathon/dtcc-ai-hackathon-2025/src/extract_pages_summary.py
...
def send_data_to_llm(chunk, number, regulation_name_for_summary_dir, base_summary_output_path="summary", use_streamlit_feedback=False):
    """
    Sends a text chunk to the LLM for summarization and saves the summary.

    Args:
        chunk (str): The text chunk to summarize.
        number (int): The chunk number (for naming the output file).
        regulation_name_for_summary_dir (str): The name of the directory (leaf) 
                                               under base_summary_output_path where this chunk's summary will be saved.
        base_summary_output_path (str): The base path where the regulation_name_for_summary_dir will be created.
        use_streamlit_feedback (bool): If True, use st.caption/st.error for feedback.
    """
...
def extract_embedded_text(pdf_path: str, use_streamlit_feedback=False) -> str:
    # ... (function remains largely the same, but can add streamlit feedback)
    if not os.path.exists(pdf_path):
        return f"Error: PDF file not found at '{pdf_path}'"
